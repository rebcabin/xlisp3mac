...
/* local prototypes */
static xlValue *GetArgStackPointer();
...
/* xGetStackPointer - built-in function '%get-stack-pointer' */
xlValue xGetStackPointer(void)
{
    xlLastArg();
    return xlMakeForeignPtr(s_stackpointer,xlCSP);
}

/* xGetStackFrame - built-in function '%get-stack-frame' */
...
/* GetArgStackPointer - get a stack pointer argument */
static xlValue *GetArgStackPointer()
{
    xlValue arg = xlGetArgForeignPtr();
    if (xlGetFPType(arg) != s_stackpointer)
        xlError("expecting stack pointer",arg);
    return (xlValue *)xlGetFPtr(arg);
}
...
void xlCallErrorHandler(void)
{
    ...
    if ((xlVal = xlGetValue(xlVal)) != xlNil)
        xlInternalCall(NULL,0,xlVal,3,xlFun,xlEnv,xlMakeForeignPtr(s_stackpointer,xlCSP));
